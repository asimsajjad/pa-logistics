...
class Accountpayable_model extends CI_Model
{
	...
	public function getAccountPayable($table, $sdate = '', $edate = '', $company = [], $factoringCompany=[], $agingSearch, $invoiceType, $invoiceNo, $carrierInvoiceRefNo, $agingFrom, $agingTo) {
		...
		$sql_documents = "SELECT did, `type`, fileurl FROM documentsOutside WHERE did IN (SELECT id FROM dispatchOutside) AND documentsOutside.type = 'carrierGd'";
	...
	public function getPayableStatementOfAccount($table,$sdate='',$edate='',$company='', $agingSearch, $invoiceType, $invoiceNo, $carrierInvoiceRefNo, $agingFrom, $agingTo) {
		...
		if ($agingSearch != '') {  
			if($agingSearch == 'fortyfive'){
				$where .= " AND DATEDIFF(CURDATE(), JSON_UNQUOTE(JSON_EXTRACT(a.dispatchMeta, '$.custInvDate'))) BETWEEN 31 AND 45";
			}elseif($agingSearch == 'sixty'){
				$where .= " AND DATEDIFF(CURDATE(), JSON_UNQUOTE(JSON_EXTRACT(a.dispatchMeta, '$.custInvDate'))) BETWEEN 46 AND 60";
			}elseif($agingSearch == 'sixtyplus'){
				$where .= " AND DATEDIFF(CURDATE(), JSON_UNQUOTE(JSON_EXTRACT(a.dispatchMeta, '$.custInvDate'))) > 60";
        ...
        $sql="Select id,pudate,dodate,pcity,dcity,rate,agentRate,parate,company,dlocation,plocation,paddressid,daddressid,dWeek,trailer,tracking,invoice,childInvoice,parentInvoice,invoiceDate,invoiceType,expectPayDate,payableAmt,payoutAmount,status,rdate,dispatchMeta, parate AS total_parate, DATEDIFF(CURDATE(), a.invoiceDate) AS days_diff, bookedUnder, bookedUnderNew
		FROM $table a
		WHERE  a.carrierPayoutDate = '0000-00-00' AND a.carrierPayoutCheck = '0' 
			AND JSON_VALID(a.dispatchMeta)
			AND JSON_UNQUOTE(JSON_EXTRACT(a.dispatchMeta, '$.custInvDate')) IS NOT NULL
		...
		foreach ($result as &$batch) {
			...
			foreach ($invoiceDetails as &$invoice) {
				...
				$invoiceDocumentsQuery = "SELECT did, type, fileurl 
                                           FROM documentsOutside 
                                           WHERE did = $invoiceId AND type = 'carrierGd'";
