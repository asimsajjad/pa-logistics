...
defined('BASEPATH') OR exit('No direct script access allowed');
	class TruckingEquipments extends CI_Controller {
		...
		function index() {
			...
			$this->load->view('admin/trucking_equipments',$data);
		...
		function add() {
			...
			if($this->input->post('save'))	{
				...
				else
				{
					...
					$res = $this->Comancontroler_model->add_data_in_table($insert_data,'truckingEquipments'); 
					if($res){
						$this->session->set_flashdata('item', 'Equipment inserted successfully.');
                        redirect(base_url('admin/trucking-equipments/update/').$res);
					}
				}
			}
        	...
        	$this->load->view('admin/layout/header');
        	$this->load->view('admin/layout/sidebar');
        	$this->load->view('admin/trucking_equipments_add');
        	$this->load->view('admin/layout/footer');
		}
		function update() {
			...
			if($this->input->post('save'))	{
                ...
                else
                {
					...
					$res = $this->Comancontroler_model->update_table_by_id($id,'truckingEquipments',$insert_data); 
					if($res){
						$this->session->set_flashdata('item', 'Equipment updated successfully.');
                        redirect(base_url('admin/trucking-equipments/update/'.$id));
					}
				}
			}
			$data['TruckingEquipments'] = $this->Comancontroler_model->get_data_by_id($id,'truckingEquipments');
			$this->load->view('admin/layout/header');
			$this->load->view('admin/layout/sidebar');
			$this->load->view('admin/trucking_equipments_update',$data);
			$this->load->view('admin/layout/footer');
		}
		function delete(){
			// if(!checkPermission($this->session->userdata('permission'),'companya')){
			...
			$id = $this->uri->segment(4);
			$result = $this->Comancontroler_model->delete($id,'truckingEquipments','id');
			if($result){
				redirect('admin/trucking-equipments');
			}
		}
	}
?>