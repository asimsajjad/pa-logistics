...
class WarehouseDispatch extends CI_Controller {
	...
	public function paWarehouseUpdate_backup() {
		...
		if($this->input->post('save'))	{
			...
			if(is_array($childInvoiceInfo)){
			    foreach($childInvoiceInfo as $ciID){
			        $ciInfo = $this->Comancontroler_model->get_data_by_column('invoice',$ciID,'warehouse_dispatch','id,childInvoice');
			        if(empty($ciInfo)) {
            ...
            else
            {
				...
				if($outwrdFilesCount > 0) {  
					...
					for($i = 0; $i < $outwrdFilesCount; $i++){
						...
						if ($this->upload->do_upload('outwrd_d'))  { 
							...
							$this->Comancontroler_model->add_data_in_table($addfile,'warehouse_documents');
							$changeField[] = array('Inward RD File','outwrdfile','Upload',$outwrd);
						}
					}
				}
				...
				$insert_data=array(
					...
					'otherChildInvoice' => $otherChildInvoice,								
						...
						foreach($disInfo as $di){
							    ...
							    if($di['childInvoice'] != '') {
							        ...
							        foreach($ciOldArray as $val){
							            if (!in_array($val, $ciNewArray)){
							                ...
							                $this->Comancontroler_model->update_table_by_column('invoice',$val,'warehouse_dispatch',$updateInvArr);
							            }
							        }
							    }
							    if($ciNewArray){
							...
							if($insert_data['childInvoice'] != '') {
								...
								foreach($ciNewArray as $subInv){
									...
									$getSubDispatch = $this->Comancontroler_model->get_data_by_column('invoice',$subInv,'warehouse_dispatch','id,dispatchMeta');
							...
							if (!empty($ociNewArray)) {
								foreach ($ociNewArray as $subInv) {
									...
									// print_r($$getSubDispatch);exit;
										$getPaentCheckboxes = $this->Comancontroler_model->get_data_by_column('invoice', $invoice, 'warehouse_dispatch', 'bol,rc,gd');
    					...
    					for($i=0;$i<count($pudate1);$i++){
				          if($pudate1[$i]!='' && $pd_type1[$i]=='pickup') {
    					    ...
    					    if($extrdispatchid1[$i] > 0) {
    					        $this->Comancontroler_model->update_table_by_id($extrdispatchid1[$i],'warehouse_dispatch_extra_info',$extraData); 
    					    } else {
						...
						for($i=0;$i<count($dodate1);$i++){
				          if($dodate1[$i]!='' && $pd_type2[$i]=='dropoff') { 
    					    ...
    					    if($extrdispatchid2[$i] > 0) {
    					        $this->Comancontroler_model->update_table_by_id($extrdispatchid2[$i],'warehouse_dispatch_extra_info',$extraData); 
    					    } else {
    					        $this->Comancontroler_model->add_data_in_table($extraData,'warehouse_dispatch_extra_info');
    					    }
				          }
		...
		$data['dispatchInfoDetails'] = $this->Comancontroler_model->get_data_by_multiple_column($where,'dispatch_info_details','*','','','');
	...
	public function index() {
		...
		if($this->input->post('generateCSV') || $this->input->post('generateXls')){
            ...
            $dispatch = $this->Comancontroler_model->downloadWarehouseDispatchCSV($sdate,$edate,$truckingCompany,$driver,$status,$invoice,$tracking);
			...
			{ 
				...
				if($this->upload->do_upload('csvfile')){
					...
					foreach ($csv as $row) { 
						...
						$check_dcity = $row[13];
						$check_dlocation = $row[14];
						$check_daddress = $row[15];
						$check_company = $row[21];
						$tracking = $row[23];
						...
						// 	continue;
						// }
						if(trim($row[35]) != '' && trim($row[35]) != '0000-00-00' && !preg_match($pattern, trim($row[35]))) {
							$data['error'][] = 'Dispatch ID '.$did.' invoice paid date (MM-DD-YYYY) format is wrong.';
							continue;
						}

						if(trim($row[36]) != '' && trim($row[36]) != '0000-00-00' && (!preg_match($pattern, trim($row[36])))) {
							$data['error'][] = 'Dispatch ID '.$did.' invoice close date (MM-DD-YYYY) format is wrong.';
							continue;
						}
						if($invoiceType != '' && $invoiceType != 'RTS' && trim($row[37]) == ''){
						...
						if(is_numeric($row[0]) && $row[0] > 1) {
							...
							if(stristr($row[24],$inv_first.''.$inv_middel)) {
								$invoice = $row[24];
							}
							elseif(strtotime($pudate) < strtotime('2024-04-25')){
								$invoice = $row[24];
						...
						$rate = str_replace('$','',$row[18]);
						if(!is_numeric($rate)) { $rate = 0; }
						$parate = str_replace('$','',$row[19]);
						if(!is_numeric($parate)) { $parate = 0; }
						...
						$insert_data = array(
							...
							'dnotes'=>$row[17],
							'invoiceNotes'=>$row[37],
							'driver_status'=>$row[32],
							'status'=>$row[31]
						);
						if(is_numeric($row[0]) && $row[0] > 1) {
							if($row[27] != 'TBD' && $row[27] != ''){
								$idate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[27]));
                                if ($idate !== false) { 
							...
							if($row[39] != '0000-00-00' && $row[39] != ''){
								$cpdate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[39]));
                                if ($cpdate !== false) { 
							...
							if($getDispatch) {
								...
								if(trim($row[34]) != ''){ 
									$irdate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[34]));
									if ($irdate !== false) { 
								...
								if(trim($row[35]) != ''){
									$ipdate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[35]));
									if ($ipdate !== false) { 
								...
								if(trim($row[36]) != ''){  
									$icdate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[36]));
									if ($icdate !== false) { 
								...
								if(trim($row[38]) != ''){  
									$carrierinvdate = DateTime::createFromFormat('m/d/Y', str_replace('-','/',$row[38]));
