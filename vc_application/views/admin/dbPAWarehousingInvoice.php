...
<div class="card mb-3">
	...
	<h3 class="m-0">DB PA Warehousing Invoices</h3>
		...
		<div class="d-flex align-items-center flex-wrap" style="gap: 15px; float: right;">
			<form class="form hide d-none" method="post" id="invoiceSearchForm" action="<?php echo base_url('Invoice/index') ?>">
				...
				<select name="dispatchType" id="dispatchType" required class="form-control" style="max-width: 150px;">
					<option value="warehouseDispatch" selected>PA Warehousing</option>
				</select>				
			</form>
					...
					<?php
						if(!empty($dispatch)){
							...
							foreach($dispatch as $key) {
								...
								if($key['invoicePaidDate'] != '') { $showAging = 'false'; }
								if($key['invoiceCloseDate'] != '') { $showAging = 'false'; }
							...
							<tr class="tr-<?php echo $key['id'];?>" <?=$bgcolor?>>
								...
								<td class="d-none"><?php //$key['invoiceDate']?>
									...
									<?php 
									if($key['invoicePaidDate'] != '') { echo '<br>Paid:&nbsp;'.$key['invoicePaidDate']; }
									if($key['invoiceCloseDate'] != '') { echo '<br>Closed:&nbsp;'.$key['invoiceCloseDate']; }
									?>
									</span>
									<span class="td-input td-input-<?php echo $key['id'];?>">
...
<script>
	$(document).ready(function() {
    	$('.select2').select2();
	  	let dispatchType = 'warehouseDispatch';
