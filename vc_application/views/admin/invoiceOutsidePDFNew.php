                            ...
                            <td align="left">
								...
								if($dispatchMeta['dispatchInfo']) { 
									...
									if (!empty($info['trailer'])) {
										$trailerRaw = array_merge($trailerRaw, explode(',', $info['trailer']));
									}
									if (!empty($childTrailer)) {
										foreach ($childTrailer as $val) {
											$trailerRaw[] = $val['trailer'];
										}
									}
									$filteredTrailers = array_filter(array_map('trim', $trailerRaw), function($val) {
										$val = strtoupper($val);
										return $val !== '' && $val !== 'N/A' && $val !== 'TBA' && $val !== 'TBD';
									});
									if (!empty($filteredTrailers)) {
										if ($trailerValLabel != '') {
                    ...
                    <table border="1">
						...
						*/ 
						...
						if($invoicePDF != 'Trucking'){ ?>
							...
							<tr><td align="center"><?php echo $puDate; ?></td><td align="center"><?=$info['invoice']?></td><td><?=$info['invoiceNotes']?></td><td align="right"></td></tr>
						...
						<?php }
							...
							<tr><td align="center"><?php echo $puDate; ?></td><td align="center"><?=$info['invoice']?></td><td><?=$info['invoiceNotes']?></td><td align="right"><?php echo '$ '.number_format($subtotal,2); ?></td></tr>
							<?=$tr?>
