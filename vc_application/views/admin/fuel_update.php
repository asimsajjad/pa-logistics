<div class="card mb-3">
 ...
 <a href="<?php echo base_url('admin/fuel');?>" class="nav-link"><input type="button" name="Back" value="Back" class="btn btn-danger"/></a>
                     </div> </div>
...
<h4><?php echo $this->session->flashdata('item'); $this->session->set_flashdata('item',''); ?></h4>
          ...
          <?php } ?>
                  ...
                  $key = $fuel[0];
		           ...
		           <form method="post" class="row" action="<?php echo base_url('admin/fuel/update/').$this->uri->segment(4);?>" enctype="multipart/form-data">
					...
					<div class="col-sm-4">	
                        <div class="form-group">
                            ...
                            <input name="amount" type="number" class="form-control" value="<?php echo $key['amount'];?>" required>
                        </div>
                     </div>
					<div class="col-sm-4">   
                        <div class="form-group">
					   ...
					   <?php if(!empty($documents)) { 
								foreach($documents as $doc) {
									if($doc['type']=='fuel') { 
										...
										<a href="'.base_url().'admin/fuel/removeotherfile/'.$doc['id'].'/'.$key['id'].'" class="remove-file" onclick="return confirm(\'Are you sure delete this file ?\')">X</a>
										<a target="_blank" href="'.base_url('assets/fuel/').''.$doc['fileurl'].'"><i class="far fa-file" style="font-size: 26px;"></i><span>'.$doc['fileurl'].'</span></a></span> &nbsp; '; 
									}
								}
							}
						?>
