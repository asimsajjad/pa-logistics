...
<div class="card mb-3">
	...
	<h3 class="m-0">Invoice</h3>
		...
		<div class="d-flex align-items-center flex-wrap" style="gap: 15px; float: right;">
			<a class="invoice-tab"  title="Pending Invoices" style="text-decoration: none; color: inherit;" href="javascript:void(0);"
			 onclick="submitInvoicesSearch('pendingInvoices')">
				<div class="invoice-top">
				<span>Pending Customer's invoices<span id="pendingInvoices_count" class="bell-number" style="color: #5bc0de;">(0)</span></span>
					<div class="bell-icon-wrapper">
					<i class="fa fa-bell bell-icon" style="font-size: 25px; margin-top: 0px; color: #5bc0de;"></i>
					</div>
				</div>
				<div class="invoice-amount text-danger" id=""><span id="pendingInvoices_total_parate" style="color: #5bc0de;">$ 0.00</span></div>
			</a>
	...
	<div class="pt-card-body">
		...
		<div class="d-block text-center">
			<form class="form form-inline pt-gap-15" id="invoiceSearchForm" method="post" action="">
				<input type="hidden" name="invoiceSearch" id="invoiceSearch" value="<?php echo $invoiceSearch; ?>">
				...
				<select name="dispatchType" id="dispatchType" required class="form-control" style="max-width: 150px;">
					...
					<option value="warehouseDispatch" <?php if($this->input->post('dispatchType') == 'warehouseDispatch') { echo 'selected'; } ?>>PA Warehousing</option>
				</select>
				...
				<select name="invoiceStatus"  class="form-control" style="max-width: 150px;">
					<option value="">Invoice Status</option>
					<option value="invoiceReady" <?php if($this->input->post('invoiceStatus') == 'invoiceReady') { echo 'selected'; } ?>>Ready</option>
					<option value="invoiced" <?php if($this->input->post('invoiceStatus') == 'invoiced') { echo 'selected'; } ?>>Invoiced</option>
					<option value="invoicePaid" <?php if($this->input->post('invoiceStatus') == 'invoicePaid') { echo 'selected'; } ?>>Paid</option>
					<option value="invoiceClose" <?php if($this->input->post('invoiceStatus') == 'invoiceClose') { echo 'selected'; } ?>>Closed</option>
				</select>
				<!-- <select name="driver" class="form-control" style="max-width: 150px;">
				...
				<?php if($this->input->post('dispatchType') == 'paDispatch' || $this->input->post('dispatchType') == 'outsideDispatch'){ ?>
					<div class="">
						<button type="button" class="btn btn-success " id="setParentBtn">Set Parent Invoice</button>
						<button type="button" class="btn btn-primary" id="makeChildBtn">Set Sub-Inovices</button>
					</div>
				<?php } ?>
			...
			<table class="table table-bordered display nowrap" id="dataTable" width="100%" cellspacing="0">
				<thead style="position:sticky;top:0">
                    <tr>
						...
						<th>Invoice #</th>
						<th>Company</th>
						<?php if(($this->input->post('invoiceSearch') == 'pendingInvoices') && ($this->input->post('dispatchType') == 'outsideDispatch')){ ?>
							<th>Carrier</th>
						<?php } ?>
						<th>Tracking #</th>
						<th>Rate</th>
				...
				<tbody>
					<?php
						if(!empty($dispatch)){
							...
							foreach($dispatch as $key) {
								...
								if($dispatchURL=='paWarehouse'){
									if($key['invoicePaidDate'] != '' && $key['invoicePaidDate'] != '0000-00-00') { $showAging = 'false'; }
									if($key['invoiceCloseDate'] != '' && $key['invoiceCloseDate'] != '0000-00-00') { $showAging = 'false'; }
								}
							...
							<tr class="tr-<?php echo $key['id'];?>" <?=$bgcolor?>>
								<td><input type="checkbox" class="invoiceChk" style="transform: scale(1.2); cursor: pointer; margin:10px;" value="<?=$key['id']?>"><?php echo $n;?></td>
								...
								<td><?php //$key['payoutAmount']?>
									<span class="td-txt td-txt-<?php echo $key['id'];?>"><?php if($key['payoutAmount'] > 0) { echo '$'; } echo '<span class="c_payoutAmount_txt_'.$key['id'].' payoutAmount">'.number_format($key['payoutAmount'],2).'</span>';?> &nbsp; <i class="fas fa-edit d-none" title="Edit" data-id="<?php echo $key['id'];?>" alt="Edit"></i> </span>
									<span class="td-input td-input-<?php echo $key['id'];?>">
								...
								<td>
									...
									<?php if($this->input->post('dispatchType') == 'paDispatch') {  ?>
										<!--a class="btn btn-sm btn-primary" href="<?php //echo base_url().'admin/'.$dispatchURL.'/update/'.$key['id'];?>?invoice">Edit Invoice</a>
										<a class="btn btn-sm btn-primary" data-id="<?=$key['id']?>" href="<?php //echo base_url('Invoice/downloadInvoicePDF/'.$key['id']);?>?invoiceWithPdf=bol-rc" target="_blank">Combine PDF</a>
										<a class="btn btn-sm btn-primary downloadPDF" data-type="dispatch" data-id="<?=$key['id']?>" href="<?php //echo base_url('Invoice/downloadInvoicePDF/'.$key['id']);?>">Download All</a>
										-->
										<!-- <a class="btn btn-sm btn-primary editInvoice" data-id="<?=$key['id']?>" data-dTable="dispatch" href="<?php echo base_url('Invoice/downloadInvoicePDF/'.$key['id']);?>" data-toggle="modal" data-target="#editInvoiceModal">Generate Invoice</a>
										
										 -->
									<?php  } 
									if($this->input->post('dispatchType') == 'outsideDispatch') {  ?>
										<!--a class="btn btn-sm btn-primary" href="<?php //echo base_url().'admin/'.$dispatchURL.'/update/'.$key['id'];?>?invoice">Edit Invoice</a>
										<a class="btn btn-sm btn-primary" data-id="<?=$key['id']?>" href="<?php //echo base_url('Invoice/downloadInvoicePDF/'.$key['id']);?>?invoiceWithPdf=bol-rc&type=outside&dTable=dispatchOutside" target="_blank">Combine PDF</a>
										<a class="btn btn-sm btn-primary downloadPDF" data-type="outside" data-id="<?=$key['id']?>" href="<?php //echo base_url('Invoice/downloadInvoicePDF/'.$key['id']);?>?dTable=dispatchOutside">Download All</a>
										-->
					...
					<tfoot>  
						<tr>
							...
							<td><strong>$<span class="rateTotal"><?php echo number_format($rate,2); ?></span></strong></td>
							<td><strong>$<span class="paRateTotal"><?php echo number_format($parate,2); ?></span></strong></td>
							<td></td>
							<td></td>
							<td></td>
							<td class="d-none"></td>
...
<div id="editInvoiceModal" class="modal fade" role="dialog" style="z-index: 10510 !important;">
	<div class="modal-dialog" style="max-width: 80% !important; z-index: 10510 !important;">
...
<script>
	...
	function submitInvoicesSearch(invoiceTypeLabel) {
		...
		if (!dispatchType) {
			alert('Please select a Division before searching invoices.');
			return; 
		}
	...
	function getInvoiceText(id) {
		return $(".c_invoice_txt_" + id).text().trim();
	}
	let parentInvoice = null;
	$("#makeChildBtn").hide();

	$("#setParentBtn").click(function() {
		let selected = $(".invoiceChk:checked").map(function(){ return $(this).val(); }).get();
		if (selected.length !== 1) {
			Swal.fire({
				title: "Invalid Selection",
				text: "Please select exactly ONE invoice to set as parent invoice.",
				icon: "warning"
			});
			return;
		}

		let invoiceId = selected[0];
		let invoiceText = getInvoiceText(invoiceId);
		Swal.fire({
			title: "Confirm Parent Invoice",
			html: "Do you want to set invoice <b>" + invoiceText + "</b> as your parent invoice?",
			icon: "question",
			showCancelButton: true,
			confirmButtonText: "Yes, set as Parent",
			cancelButtonText: "No"
		}).then((result) => {
			if (result.isConfirmed) {
				parentInvoice = invoiceId;
				$(".invoiceChk").removeClass("parent-invoice");
				$("label").removeClass("parent-invoice-label");
				let $parentChk = $(".invoiceChk[value='" + parentInvoice + "']");
				$parentChk.addClass("parent-invoice");
				$parentChk.closest("label").addClass("parent-invoice-label");
				$("#makeChildBtn").show();
			}
		});
	});

	$(document).on("change", ".invoiceChk", function() {
		if (parentInvoice) {
			let $parentChk = $(".invoiceChk[value='" + parentInvoice + "']");
	...
	$("#makeChildBtn").click(function() {
		if (!parentInvoice) {
			Swal.fire({
				title: "No Parent Selected",
				text: "Please set a parent invoice first.",
				icon: "warning"
			});
			return;
		}
		...
		let selected = $(".invoiceChk:checked").map(function(){ return $(this).val(); }).get();
		selected = selected.filter(id => id !== parentInvoice); 

		if (selected.length === 0) {
			Swal.fire({
				title: "No Sub-Invoice Selected",
				text: "Please select at least one invoice as sub-invoice.",
				icon: "warning"
			});
			return;
		}
		...
		if (!dispatchType) {
			Swal.fire({
				title: "Missing Dispatch Type",
				text: "Please select a dispatch type before setting sub-invoices.",
				icon: "warning"
			});
			return;
		}

		let parentInvoiceText = getInvoiceText(parentInvoice);
    	let childInvoiceTexts = selected.map(id => getInvoiceText(id));

		Swal.fire({
			title: "Confirm Sub-Invoices",
			html: "Do you want to set the invoices:<br><b>" + childInvoiceTexts.join(", ") + "</b><br>as sub-invoices of parent invoice: <b>" + parentInvoiceText + "</b>?",
			icon: "question",
			showCancelButton: true,
			confirmButtonText: "Yes, set as Sub-Invoice",
			cancelButtonText: "No"
		}).then((result) => {
			if (result.isConfirmed) {
				$.post("<?=base_url('Invoice/setSubInvoices')?>", {
					parentInvoiceId: parentInvoice,
					childInvoiceIds: selected,
