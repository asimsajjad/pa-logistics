...
<div class="card mb-3" style="padding :25px;">
	<div class="pt-card-header pt-card-header pt-border-bottom d-flex align-items-center justify-content-between bg-transparent">
    	<h3 class="mb-0">Warehouse Outbounds</h3>
    	<div class="add_page  d-flex" style="float: right;">
			<a class="nav-link p-0" title="Create Section" href="<?php echo  base_url().'admin/warehouse/addOutbounds';?>"><input type="button" name="add" value="Add New" class="btn btn-success pt-cta"/>
			</a>  
			<a class="nav-link p-0 ml-1" title="Upload Inbounds" href="<?php echo  base_url().'admin/warehouse/uploadOutbounds';?>" style="display: inline;"><input type="button" name="add" value="Upload Outbounds" class="btn btn-primary pt-cta"/>
			</a>
    	</div>
	</div>
					...
					<?php if (!empty($warehouse)): 
						...
						<?php 
							...
							<!-- <tr class="parent-row" data-details='<?php echo json_encode($summary["details"]); ?>'> -->
								...
								<td>
									<a class="btn btn-sm btn-danger" href="<?php echo base_url('Warehouse/downloadBoundPDF/'.$summary['id']);?>?date=<?=$summary['dated'] ?>&customerId=<?=$summary['customer_id'] ?>&warehouseId=<?=$summary['warehouse_id'] ?>&sublocationId=<?=$summary['sublocation_id'] ?>&type=outbound">Get Outward DD</a> 
								</td>
							</tr>
	...
	function formatChildRows(details) {
		...
		details.forEach(function(detail, index) {
			html += '<tr>' +
				...
				'<td style="">' +
					'<a class="btn btn-sm btn-success" target="_blank" href="' + baseUrl + 'admin/warehouse/updateOutbounds/'+ detail.id + '?dated=' + encodeURIComponent(detail.dated) + '">Edit</a> ' +
					'<a class="btn btn-sm btn-danger" href="' + baseUrl + 'admin/warehouse/deleteOutbounds/' + detail.id + '" onclick="return confirm(\'Delete this item?\')">Delete</a>' +
				'</td>' +
			'</tr>';
		});
