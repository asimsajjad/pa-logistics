				...
				<?php
					...
					<form method="post" action="">
						...
						<div class="row">
							<div class="col-sm-4 col-md-4 col-xl-2 p-0">
								<div class="form-group">
									...
									<input readonly class="form-control" type="date" name="dated" value="<?php echo date($dated); ?>"/>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-sm-4 col-md-4 col-xl-2 p-0">
								<div class="form-group">
									<label for="contain">New Date Out/In</label>
									<input class="form-control" type="date" name="dated_new" value="<?php echo date('Y-m-d'); ?>" required/>
								</div>
							</div>
							<div class="col-sm-4 col-md-4 col-xl-3 p-0">
								<div class="form-group">
									<label for="contain">New Warehouse</label>
									<select class="form-control select2" name="warehouse_id_new" id="" required>
										<option value="">Select Warehouse</option>
										<?php  
										if (!empty($warehouse_address)) {
											foreach ($warehouse_address as $val) {
												echo '<option value="' . $val['id'] . '">' . $val['warehouse'] . ' (' . $val['address'] . ')</option>';
											}
										}
										?>
									</select>
							...
							<!-- <div class="col-sm-4 col-md-4 col-xl-3 p-0">
								<div class="form-group">
									<label for="contain">New Sublocation</label>
									<select class="form-control select2" name="sublocation_id_new" id="newSublocationDropdown" required>
							...
							<div class="col-sm-4 col-md-4 col-xl-3 p-0">
								<div class="form-group">
									<label for="contain">New Sublocation</label>
									<select class="form-control select2" name="sublocation_id_new" id="sublocation-dropdown-new" required>
										<option value="">Select Sublocation</option>
									</select>
								</div>
							</div>
						...
						<div class="material-group row">
							...
							<div class="col-xl-1 p-0">
								<label>Lot Pos.</label>
								<input type="hidden" name="lotNumber[]" value="<?= $material['lot_number'] ?>" class="form-control" />
								<input name="lotNumberNew[]" value="" class="form-control" />
							</div>
							<div class="col-xl-1 p-0">
								<label>Pallet #</label>
							...
							<div class="col-xl-1 p-0">
								...
								<input type="hidden" name="palletPosition[]" value="<?= $material['pallet_position'] ?>" class="form-control" />
								<input name="palletPositionNew[]" value="" class="form-control" />
							</div>
							<div class="col-xl-1 p-0">
							...
							<div class="col-xl-12 col-md-12 col-sm-12 p-0" style="margin-top: 0px;">
								<div class="form-group">
									<textarea name="notes[]" class="form-control notes" rows="1" style="height:48px; overflow: hidden;" placeholder="Enter notes"><?php echo $material['notes']; ?></textarea>
								</div>
							</div>
						</div>
						<?php endforeach; ?>
						<div class="form-group">
							<input type="submit" name="save" value="Transer" class="btn btn-success mt-2"/>
						</div>
					</form>
					<?php
				?> 
...
$(document).ready(function () {
	...
	$('select[name="warehouse_id_new"]').on('change', function() {
		...
		if(warehouseId) {
			$.ajax({
				...
				success: function(data) {
					...
					$('#sublocation-dropdown-new').html(options);
				}
			});
		} else {
			$('#sublocation-dropdown-new').html('<option value="">Select Sublocation</option>');
		}
	});
});
</script>