			...
			<input type="text" required readonly placeholder="Start Date" value="<?php echo $sdate; ?>" name="sdate" style="width: 120px;" class="form-control datepicker"> &nbsp;
				...
				<select name="company[]" class="form-control select2" multiple="multiple" data-placeholder="Select a Customer" style="max-width: 250px;">
					...
					?>
					 ...
					 &nbsp; 
              <select name="material_id[]" class="form-control select2" multiple="multiple" data-placeholder="Select a Material" style="max-width: 250px;">
					<option value="">All Materials</option>
					<?php 
						$selected_material = $this->input->post('material_id');
						$materialArr = array();
						if(!empty($materials)){
							foreach($materials as $val){
								$materialArr[$val['id']] = $val['material_id'];
								echo '<option value="'.$val['id'].'"';
          ...
          <tr>
            ...
            if(!empty($warehouse)){
              ...
              foreach($warehouse as $key)
              {
				$note='';
				// print_r($key);exit;
                if($key['type']=='warehouseInbounds' || $key['type']=='inbound'){
                	$note='Inbounds';
                }
                if($key['type']=='warehouseOutbounds' || $key['type']=='outbound'){
                  $note='Outbounds';
                }
            ?>
