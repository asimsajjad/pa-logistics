					...
					<?php if (!empty($warehouse)): 
						...
						foreach ($warehouse as $summary): ?>
							<tr class="parent-row" data-details='<?php echo json_encode($summary["details"]); ?>'>
								...
								<td style=""><?php echo $summary['dateOut']; ?></td>
								...
								<td>
									<a class="btn btn-sm btn-danger" href="<?php echo base_url('Warehouse/downloadBoundPDF/'.$summary['id']);?>?date=<?=$summary['dateOut'] ?>&customerId=<?=$summary['customerId'] ?>&warehouseId=<?=$summary['warehouseAddressId'] ?>&sublocationId=<?=$summary['sublocationId'] ?>&dTable=warehouseOutbounds">Get Outward DD</a> 
								</td>
							</tr>
	...
	function formatChildRows(details) {
		...
		details.forEach(function(detail, index) {
			html += '<tr>' +
				...
				'<td style="">' +
					'<a class="btn btn-sm btn-success" target="_blank" href="' + baseUrl + 'admin/warehouse/updateOutbounds/' + detail.id + '">Edit</a> ' +
