...
$(document).ready(function () {
	...
	function fetchPalletNumbers($group) {
		...
		const warehouseAddressId = $('select[name="warehouseAddressId"]').val();

		if (materialId && batch && warehouseAddressId) {
			$.ajax({
				url: "<?php echo base_url('Warehouse/getPalletsByMaterialAndBatch'); ?>",
				type: 'POST',
				data: { materialId, batch, warehouseAddressId},
				dataType: 'json',
				success: function (response) {
					if (response.status === 'success') {
	...
	$(document).on('change', '.palletNumberDropdown', function () {
		...
		if (lotNumber !== undefined) {
			$group.find('.lotNumber').val(lotNumber);
		}
		if (palletQty !== undefined) {
			$group.find('.palletQuantity').val(palletQty);
		}
		if (piecesQty !== undefined) {
			$group.find('.piecesQuantity').val(piecesQty);
		}
		if (palletPosition !== undefined) {
			$group.find('.palletPosition').val(palletPosition);
		}
	});
